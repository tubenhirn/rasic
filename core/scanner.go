...
func createLocalTempfile(client types.HTTPClient, source plugins.Source, project types.RasicProject, fileName string, defaultBranch string, authToken string, tmpDir string) (string, error) {
	// TODO: rework this to be more dynamic...
	var fileString string
	if project.ProjectType == "github" {
		fileString = source.GetFile(client, project.FullName, fileName, defaultBranch, authToken)
	} else if project.ProjectType == "gitlab" {
		fileString = source.GetFile(client, strconv.Itoa(project.ID), fileName, defaultBranch, authToken)
	} else {
		return "", errors.New("no ProjectType set")
	}
	...
	pterm.Info.Println("no " + fileName + " file in project - skip downloading")
...
func RepositoryScanner(client types.HTTPClient, source plugins.Source, project types.RasicProject, token string, knownIssues []types.RasicIssue, minSeverity types.Severity) ([]types.RasicIssue, error) {
	...
	if dirErr != nil {
		// return nil, errors.New("Error creating local tmp dir.")
		pterm.Warning.Println("dir exists")
	}

	// find path to trivy binary
	binary, lookErr := exec.LookPath("trivy")
	if lookErr != nil {
		return nil, errors.New("Error looking up trivy binary.")
	}
	...
	if project.IgnoreFileName != "" {
		ignorefilePath, _ := createLocalTempfile(client, source, project, project.IgnoreFileName, project.DefaultBranch, token, resultFileDir)
	...
	if execErr != nil {
		return nil, errors.New("Error executing trivy scan.")
	}

	repoResult, err := os.ReadFile(resultfilePath)
	if err != nil {
		return nil, errors.New("Error reading scan result.")
	}
	...
	if unmarshalerr != nil {
		...
		return nil, errors.New("Error unmarshaling report.")
	}
...
func ContainerRegistryScan(httpClient types.HTTPClient, apiPlugin plugins.Source, project types.RasicProject, userName string, authToken string, newIssues []types.RasicIssue, severity types.Severity, registryExcudePattern string) []types.RasicIssue {
	...
	configfilePath, _ := createLocalTempfile(httpClient, apiPlugin, project, configfileName, project.DefaultBranch, authToken, resultFileDir)
